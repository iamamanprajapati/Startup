...
const QueryCard = ({data,onDelete}) => {
  ...
  return (
    ...
    <span className='text-sm text-gray-600'>Date : {deteFormater(data.createdAt)}</span>
     ...
     <span onClick={updateQueryStatus} className={` ${status ? 'bg-green-400 text-black' : "bg-red-400"} cursor-pointer font-semibold text-sm w-fit px-4 py-1 mt-2 rounded hover:scale-110 transition`}>
