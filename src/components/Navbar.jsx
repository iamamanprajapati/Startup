...
const Navbar = () => {
  ...
  return (
    <div
      className={`w-full bg-primary fixed top-0 left-0 right-0 z-50 transition-transform duration-500  ${
        isSticky ? "fixed top-0 border-b-[1px] border-white/5 " : "relative"
    ...
    >
      <div className={`max-w-7xl mx-auto py-3  px-6  `}>
        <div className="flex justify-between items-center">
              ...
              {NavbarMenu.map((menu, index) => (
                <li
                  ...
                  className={`relative group cursor-pointer
                        ${
                          NavbarMenu.length - 1 === index &&
                          "bg-button hover:bg-button/90 transition-all hover:-translate-y-1 duration-300 px-4 py-1.5 rounded-md text-white capitalize"
                        }
                    `}
                >
                  ...
                  <span
                    className={` ${
                      NavbarMenu.length - 1 !== index
                        ? "absolute left-0 bottom-0 h-0.5 w-0 bg-orange-500 transition-all duration-300 group-hover:w-full "
                        : ""
                    }`}
                  ></span>
                </li>
          ...
          {/* menu button  */}
          <div
            className="flex md:hidden text-white text-2xl font-semibold transition-all duration-500"
            onClick={handleMenuOpen}
          >
            {showMenu ? (
              <RiCloseFill fontWeight={900} />
            ) : (
